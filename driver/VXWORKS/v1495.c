...
****************************************************************************/
int
write_flash_page(unsigned int addr, unsigned char *page, int pagenum, int flag)
{
...
****************************************************************************/
int
read_flash_page(unsigned int addr, unsigned char *page, int pagenum, int flag)
{
  volatile V1495 *v1495 = (V1495 *) addr;
  int i, flash_addr, data;
  /*volatile*/ 
...
v1495firmware(0x10100000,"V1495USER1.0.rbf",0,0)
...
user_vme = 0; /* ONLY USER !!!!!!!!!!!! */
  ...
  printf("*   Roman Pomatsalyuk: Moller version 18-May-2007      *\n");
  printf("********************************************************\n\n");
  ...
  while(!finish)
  {
    ...
    {
      write_flash_page(baseaddr, pdw, pa, user_vme);
      read_flash_page(baseaddr, pdr, pa, user_vme);
      for(i=0; i<PAGE_SIZE; i++)
      {
        if(pdr[i] != pdw[i])
        {
          printf("\n[%3d] written 0x%02x, read back 0x%02x",i,pdw[i],pdr[i]);
...
int
v1495fscan(unsigned int baseaddr, char *filename)
{
  int finish,i;
  int bp, bcnt, pa;
  char c;
...
user_vme = 0; /* ONLY USER !!!!!!!!!!!! */
  ...
  if(user_vme == 0) /* FPGA "User" */
  {
    ...
    printf("Scaning firmware of the FPGA USER with the file %s\n",filename);
  }
  ...
  while(!finish)
  {
    ...
    if((bp == PAGE_SIZE) || finish)
    {
      ...
      for(i=0; i<PAGE_SIZE; i++)
      {
        ...
        {
          printf("\n[%3d] written 0x%02x, read back 0x%02x",i,pdw[i],pdr[i]);
          printf(" -> Flash checking failure (byte %d of page %d)!\n",i,pa);
	  //          printf("\nFirmware not loaded !\n");
          return (-1);
        }
	  }
